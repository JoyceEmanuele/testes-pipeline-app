...
export async function w_insert(qPars: {
  ...
  if (qPars.CHILLER_CARRIER_MODEL_ID !== undefined) fields.push('CHILLER_CARRIER_MODEL_ID')

  const sentence = `INSERT INTO CHILLERS (${fields.join(', ')}) VALUES(:${fields.join(', :')})`

  if (operationLogData) {
    await saveOperationLog('CHILLERS', sentence, qPars, operationLogData);
    dbLogger('CHILLERS', sentence, qPars, operationLogData);
  }
...
export async function w_updateInfo(
  ...
  if (qPars.CHILLER_CARRIER_MODEL_ID !== undefined) { fields.push('CHILLER_CARRIER_MODEL_ID = :CHILLER_CARRIER_MODEL_ID') }

  if (!fields.length) throw Error('No fields to update').HttpStatus(500).DebugInfo({ qPars })

  const sentence = `UPDATE CHILLERS SET ${fields.join(', ')} WHERE ID = :ID`
...
export async function w_updateInfoByAssetId(
  ...
  const sentence = `UPDATE CHILLERS SET ${fields.join(', ')} WHERE ASSET_ID = :ASSET_ID`
...
export async function w_delete(qPars: { ID: number }, operationLogData: OperationLogData) {
  ...
  const sentence = `DELETE FROM CHILLERS WHERE CHILLERS.ID = :ID`;
...
export async function getAssetChillerByAssetId(qPars: { ASSET_ID: number }) {
  const sentence = `
    SELECT
      CHILLERS.ID AS CHILLER_ID
    FROM 
      CHILLERS
    WHERE CHILLERS.ASSET_ID = :ASSET_ID
  `
...
export async function w_deleteByAssetId(qPars: { ASSET_ID: number }, operationLogData: OperationLogData) {
  const sentence = `DELETE CHILLERS FROM CHILLERS WHERE CHILLERS.ASSET_ID = :ASSET_ID`;
