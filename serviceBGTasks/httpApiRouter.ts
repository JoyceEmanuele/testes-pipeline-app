...
import * as devsCfg from "./devsCfg/devsCfg";
...
const internalRoutes: API_Internal = {
  ['/diel-internal/bgtasks/addDeviceToOtaQueue']: fwMonitor.addDeviceToOtaQueue,
  ['/diel-internal/bgtasks/getDevsCfg']: devsCfg.getDevsCfg,
  ['/diel-internal/bgtasks/checkOtaQueue']: fwMonitor.checkOtaQueue,
};
...
async function confApm (user: string, perfil: string) {
  if(user  != undefined){
    ...
    apmService.setCustomUserApm(user, clientsString, perfil);
  }
  else {
    apmService.setCustomUserApm("(no session)", "(no session)", "(no session)");
  }
}
...
const expressErrorHandler: express.ErrorRequestHandler = function (err, req, res, _next) {
  try {
    ...
    logger.error({
      message: `[BG-TASKS] There was an error processing route ${req.originalUrl}`,
      url: req.originalUrl,
      user: user,
      parameters: dbgPars,
      err: err
